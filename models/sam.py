...
#     sam_model = SamModel.from_pretrained("facebook/sam-vit-large").to(torch_device)
...
#     print("sam_processor: ", sam_processor)
#     print("sam_model_dict: ",sam_model_dict)
...
def load_sam():
    try:
        ...
        sam_model = sam_model.to(torch_device)
        
        print("Loading SAM processor...")
        sam_processor = SamProcessor.from_pretrained("facebook/sam-vit-large")
        if sam_processor is None:
            raise RuntimeError("Failed to load SAM processor")
            
        # Verify the model and processor are loaded correctly
        if not isinstance(sam_model, SamModel) or not isinstance(sam_processor, SamProcessor):
            raise RuntimeError("Invalid SAM model or processor type")
        ...
        if not all(key in sam_model_dict for key in ['sam_model', 'sam_processor']):
            raise RuntimeError("Missing required keys in SAM model dictionary")
...
def sam(sam_model_dict, image, input_points=None, input_boxes=None, target_mask_shape=None, return_numpy=True):
    """target_mask_shape: (h, w)"""
    # Add validation for sam_model_dict
    if sam_model_dict is None:
        raise ValueError("SAM model dictionary is None")
    
    if not isinstance(sam_model_dict, dict):
        raise ValueError("SAM model dictionary must be a dictionary")
        
    if 'sam_model' not in sam_model_dict or 'sam_processor' not in sam_model_dict:
        raise ValueError("Invalid SAM model dictionary structure - missing required keys")
        
    sam_model = sam_model_dict.get('sam_model')
    sam_processor = sam_model_dict.get('sam_processor')
    
    # Validate both components are present
    if sam_model is None or sam_processor is None:
        raise ValueError("SAM model or processor is None")
    
    # Handle input boxes conversion
    if input_boxes and isinstance(input_boxes[0], tuple):
        # Convert tuple to list
        input_boxes = [list(input_box) for input_box in input_boxes]
    ...
    try:
        ...
        print(f"Error during SAM processing: {str(e)}")
        raise
...
def sam_box_input(model_dict, image, input_boxes, **kwargs):
    """
    Helper function for box input.
    """
    if model_dict is None:
        raise ValueError("Model dictionary is None")
        
    # Get the SAM model dictionary from the global model dictionary
    sam_model_dict = model_dict.get('sam_model_dict')
    if sam_model_dict is None:
        raise ValueError("SAM model dictionary not found in model_dict")
...
def sam_refine_attn(input_boxes_w, obj_id, sam_input_image, token_attn_np, model_dict, height, width, H, W, use_box_input, gaussian_sigma, mask_th_for_box, n_erode_dilate_mask_for_box, mask_th_for_point, discourage_mask_below_confidence, discourage_mask_below_coarse_iou, verbose):
    ...
    if model_dict is None:
        raise ValueError("Model dictionary is None in sam_refine_attn")
    
    if not isinstance(model_dict, dict):
        raise ValueError("Model dictionary must be a dictionary")
